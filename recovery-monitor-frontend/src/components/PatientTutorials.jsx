    ...
    <Panel title={<><BookOpenText size={16} /> Your exercise tutorials</>} subtitle="Tutorials approved by your physiotherapist">
      ...
      <div className="tutorial-list">
        {tutorials.data?.map((tutorial) => {
          const videoUrl = tutorial.media?.url || tutorial.reference_video?.url || `/api/reference-videos/${tutorial.reference_video_id}/video`;
          return <article className="tutorial-card" key={tutorial.id}>
            <div className="tutorial-meta"><span className="badge badge-green">Approved</span><strong>{exerciseName(tutorial.exercise)}</strong><span>{tutorial.target_reps} repetitions{tutorial.target_depth_deg ? ` · ${tutorial.target_depth_deg}° depth` : ''}</span></div>
            {videoUrl && <div className="reference"><span className="mini-label"><Video size={13} /> {tutorial.media?.status === 'ready' ? 'Generated tutorial video' : (tutorial.reference_video?.title || 'Reference video')}</span><video src={videoUrl} controls playsInline preload="metadata" /></div>}
